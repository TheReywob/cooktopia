...
<body class="u-body u-stick-footer">
  ...
  <section class="u-align-center u-clearfix u-custom-color-1 u-section-1" id="sec-a638">
    <div class="u-clearfix u-sheet u-sheet-1">
      <h1 class="u-text u-text-default u-text-1">My Recipes</h1>
    </div>
  </section>
          ...
          <div class="u-layout-col">
            <div class="u-size-30">
              <div class="u-layout-row">
                ...
                // Connect to database
                require "connect.php";
                // Select user created entries from recipes (newest to oldest)
                $sql = "select * from recipes where userId = '$userId' order by eDate desc";
                $result = $connect->query($sql);
                // If the user has no recipes
                if ($result->num_rows == 0){
                  echo "<h3>It looks like you don't have any recipes yet.</h3>";
                }
                ...
                echo '<img class="u-align-center u-image u-image-round u-radius-10 u-image-1" src="images/recipe_images/' . $row['eImage'] . '" onerror=this.src="images/recipe_images/sample.png" data-image-width="728" data-image-height="1092">';
                ...
                // Print recipe name
                echo '<h2 class="u-align-center u-text u-text-default u-text-2">'.$row['eTitle'].'</h2>';

                // Print recipe description
                echo '<h4 class="u-align-center u-text u-text-default u-text-3">'.$row['eDescription'].'</h4>';
                ...
                echo '<span style="font-weight: 700;">'.$row['eSkillLevel'].'/5</span>';
                echo '</h4>';

                // Create See Recipe button
                echo '<form action="get_recipe.php" method="post">';
                echo '<input name="submit[' . $row['eId'] . ']" type="submit" value="See Recipe" class="u-align-center u-border-3 u-border-palette-3-base u-btn u-btn-round u-button-style u-hover-palette-1-light-1 u-palette-1-base u-radius-50 u-btn-1" style="border-width:3px">';
                echo '</form>';

                // Create Edit Recipe button
                echo '<form action="edit_recipe.php" method="post">';
                echo '<input name="submit[' . $row['eId'] . ']" type="submit" value="Edit Recipe" class="u-align-center u-border-3 u-border-palette-3-base u-btn u-btn-round u-button-style u-hover-palette-1-light-1 u-palette-1-base u-radius-50 u-btn-1" style="border-width:3px">';
                echo "</form>";

                // Create Delete Recipe button
                ?>
                <form action="delete_recipe.php" method="post" onsubmit="return confirm('Are you sure you want to delete <?=$row['eTitle']?>?\n\nThis action is permanent and cannot be reversed.');">
                <?php
                echo '<input name="submit[' . $row['eId'] . ']" type="submit" value="Delete Recipe" class="u-align-center u-border-3 u-border-palette-3-base u-btn u-btn-round u-button-style u-hover-palette-1-light-1 u-palette-1-base u-radius-50 u-btn-1" style="border-width:3px">';
                echo "</form>";

                // Print recipe date of creation
                echo '<h4 class="u-align-center u-text u-text-default u-text-grey-15 u-text-5">Uploaded on '.$row['eDate'].'</h4>';
